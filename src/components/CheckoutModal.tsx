...
export const CheckoutModal: React.FC<CheckoutModalProps> = ({
  ...
  const handleSubmitOrder = async (e: React.FormEvent) => {
    ...
    if (!customerName.trim()) {
      setErrorMessage('Please enter your full name for courier delivery.');
      return;
    }
    if (!phone.trim() || phone.trim().length < 11) {
      setErrorMessage('Please provide a valid 11-digit Bangladesh mobile number for delivery confirmation.');
      return;
    }
    if (!address.trim()) {
          ...
          {!currentUser && (
            <div className="p-3 bg-amber-50 border border-amber-200 rounded-2xl flex items-center justify-between gap-3 text-xs">
              <div className="flex items-center gap-2 text-amber-900">
                ...
                <span>You need to be logged in to complete checkout and track order history.</span>
              </div>
              <button
                type="button"
                onClick={onRequireLogin}
                className="px-3 py-1.5 bg-amber-500 hover:bg-amber-600 text-stone-950 font-bold rounded-xl shrink-0 transition-colors"
              >
                Sign In Now
              </button>
            </div>
